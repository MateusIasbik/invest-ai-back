...
describe("POST /", () => {
  ...
  it("should update existing user money and assets", async () => {
    ...
    expect(user!.money).toBe(updatedData.money);
    expect(user!.assets[0].amount).toBe(updatedData.assets[0].amount);
  });

  it("should create user without assets", async () => {
    const userData = generateUserData();
    userData.assets = [];

    const res = await api.post("/").send(userData);
...
describe("GET /:frontId", () => {
    it("should return user data if frontId exists", async () => {
        ...
        expect(res.body.money).toBeCloseTo(userData.money, 2);
